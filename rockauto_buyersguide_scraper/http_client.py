"""HTTP lookup helpers with rate-limit backoff."""
...
def _simulate_rate_limit(target: str) -> None:
    if "rate_limit" in target.lower():
        raise RateLimitError("HTTP rate limited")


async def _perform_http_request(target: str) -> Dict[str, Any]:
    _simulate_rate_limit(target)
    return {"status": "ok", "data": f"fetched:{target}"}
...
async def fetch_http_data(
    ...
    *,
    ...
    async def operation() -> Dict[str, Any]:
        return await _perform_http_request(target)
